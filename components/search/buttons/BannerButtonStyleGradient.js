...
const BannerButtonStyleGradient = ({ data, onButtonPress, buttonStyle }) => {
  const startColor = COLORS.grey_0;
  const middleColor = COLORS.grey_0;
  const endColor = COLORS.grey_1_t;

  return (
    <TouchableOpacity onPress={() => onButtonPress(data.search)} style={[buttonStyle, {position: 'relative'}]}>
...
export default BannerButtonStyleGradient;
